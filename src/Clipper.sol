...
import "./interfaces/ISpotter.sol";
import "./interfaces/IAbacus.sol";
import "./interfaces/IClipperCallee.sol";
...
contract Clipper {
    ...
    IVat  immutable public vat;   // Core CDP Engine

    IDog     public dog;      // Liquidation module
    address     public vow;      // Recipient of dai raised in auctions
    ISpotter public spotter;  // Collateral price module
    IAbacus  public calc;     // Current price calculator
    ...
    function file(bytes32 what, address data) external auth lock {
        if (what == "spotter") spotter = ISpotter(data);
        else if (what == "dog")    dog = IDog(data);
        else if (what == "vow")    vow = data;
        else if (what == "calc")  calc = IAbacus(data);
        else revert("Clipper/file-unrecognized-param");
        emit File(what, data);
    }
