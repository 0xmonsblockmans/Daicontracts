...
import "./interfaces/IFlop.sol";
import "./interfaces/IFlap.sol";
...
contract Vow {
    ...
    IFlap public flapper;   // Surplus Auction House
    IFlop public flopper;   // Debt Auction House
    ...
    constructor(address vat_, address flapper_, address flopper_) {
        wards[msg.sender] = 1;
        vat     = IVat(vat_);
        flapper = IFlap(flapper_);
        flopper = IFlop(flopper_);
        vat.hope(flapper_);
        live = 1;
    }
    ...
    function file(bytes32 what, address data) external auth {
        ...
        else if (what == "flopper") flopper = IFlop(data);
        else revert("Vow/file-unrecognized-param");
    }
