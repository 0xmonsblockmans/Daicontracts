...
contract StandardToken is Token {
    ...
    function allowance(
        address _owner,
        address _spender
    ) public override returns (uint256 remaining) {
...
contract BAToken is StandardToken, SafeMath {
    ...
    // constructor
    constructor(address _ethFundDeposit, address _batFundDeposit) {
    ...
    function finalize() external {
        ...
        // move to operational
        isFinalized = true;
        if (!payable(ethFundDeposit).send(address(this).balance)) revert(); // send the eth to Brave International
    }
